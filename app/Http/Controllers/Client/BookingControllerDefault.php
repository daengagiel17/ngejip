...
class BookingController extends Controller
{
    ...
    public function cekHarga(Request $request)
    {
        $request->validate([
            ...
            'jenis_trip.required' => 'Jenis trip is required',
        ...
        $paket_trip = "sunrise";
        if($request->jenis_trip == "private"){
            $paket_trip = $request->paket_trip;
        }
        // Membuat record harga detail
        $harga_detail = new HargaDetail;

        // Mengambil data harga change
        $harga_jeep = HargaJeepChanged::whereDate("harga_jeep_changed_tgl_start", "<=", $tanggal_booking)->whereDate("harga_jeep_changed_tgl_finish", ">=", $tanggal_booking)
            ->where('harga_jeep_changed_paket', $paket_trip)->where('harga_jeep_changed_jenis', $request->jenis_trip)
            ->first();

        // Jika tidak ada maka mengambil di harga default
        if(empty($harga_jeep)){
            $harga_jeep = HargaJeepDefault::orderByDesc('harga_jeep_default_pertanggal')->whereDate('harga_jeep_default_pertanggal', '<=', $tanggal_booking)
            ->where('harga_jeep_default_paket', $paket_trip)->where('harga_jeep_default_jenis', $request->jenis_trip)
            ->first();
        ...
        $booking->booking_jenis_trip = $request->jenis_trip;
    ...
    private function cekSlot($tanggal){
        ...
        $slot = $jmlDriver - $jmlSettlement - $jmlPending - $jmlCapture - $jmlDriverOff;
